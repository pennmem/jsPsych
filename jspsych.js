...
jsPsych.pluginAPI = (function() {
  ...
  var root_keydown_listener = function(e){
    for(var i=0; i<keyboard_listeners.length; i++){
      keyboard_listeners[i].fn(e, true);
    }
    held_keys[e.key] = true;
  }
  var root_keyup_listener = function(e){
    for(var i=0; i<keyboard_listeners.length; i++){
      keyboard_listeners[i].fn(e, false);
    }
    held_keys[e.key] = false;
  }

  module.getHeldKeys = function() {
    return held_keys; // promise not to touch this
  }
  ...
  module.getKeyboardResponse = function(parameters) {
    ...
    var listener_function = function(e, down) {
      var key_time;
      if (parameters.rt_method == 'performance') {
      ...
      if(down) {
        ...
        if (((typeof parameters.record_up == 'undefined') || !parameters.record_up)) {
          valid_response = false;
        }
      }
      ...
      if (valid_response) {
        ...
        if(parameters.propagate !== 'undefined' && !parameters.propagate)
          e.preventDefault();

        keep = parameters.callback_function({
          key: e.key,
          rt: key_time - start_time,
          down: down
        });

        if(keep === false || (parameters.propagate !== 'undefined' && !parameters.propagate)) {
          e.preventDefault();
        }
      }
    };
